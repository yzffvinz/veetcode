...
var totalFruit = function (fruits) {
    ...
    const blockWeighs = [];
    let [slow, fast] = [0, 0];
    while (slow < fruits.length && fast < fruits.length) {
        blocks.push(fruits[slow]);
        fast = slow;
        let counter = 0;
        while (fruits[slow] === fruits[fast]) {
            fast++;
            counter++;
        }
        blockWeighs.push(counter);
        slow = fast;
    }
    ...
    while (i < blocks.length && j < blocks.length) {
        if (baskets.includes(blocks[j])) {
            count += blockWeighs[j];
            j++;
        } else if (baskets.length < 2) {
            baskets.push(blocks[j]);
            count += blockWeighs[j];
            j++;
        } else {
            i = --j;
